...
void vma_unmap(pagetable_t pagetable, uint64 va, uint64 nbytes, struct vma *v)
{
    uint64 addr;
    pte_t *pte;
    for (addr = va; addr < va + nbytes; addr += PGSIZE)
    {
        ...
        if (*pte & PTE_V)
        {
            ...
            if ((*pte & PTE_D) && (v->flags & MAP_SHARED))
            {
                begin_op();
                ilock(v->f->ip);
                uint64 offset = addr - v->start;
                if (offset < 0)
                {
                    writei(v->f->ip, 0, pa - offset, v->offset, PGSIZE + offset);
                }
                else if (offset + PGSIZE > v->sz)
                {
